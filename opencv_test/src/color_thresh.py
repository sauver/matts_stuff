...
#determine the objects moments and check that the area is large
    ...
    # [row, col] =cv.GetSize(thresholded_img)
    #mat = cv.CreateMat(row,col, cv.CV_32FC1)
    #mat=cv.GetMat(thresholded_img)
    #[row, col] =cv.GetSize(thresholded_img_y)
    #mat_y = cv.CreateMat(row,col, cv.CV_32FC1)
    #mat_y=cv.GetMat(thresholded_img_y)i
    #mat = np.asarray(mat)
    #mat_y = np.asarray(mat_y)
    cv.Erode(thresholded_img_y,thresholded_img_y,None,4)
    cv.Dilate(thresholded_img_y, thresholded_img_y,None,4)
    cv.Erode(thresholded_img, thresholded_img,None,4)
